...
#cat $1 | xargs -I{fileID} sh -c 'fastq-dump --outdir /home3/acobian/Marisa/V2 --gzip --skip-technical --readids --dumpbase --split-files --clip {fileID}'
...
##this one done with split_blast_queries

##merge all .blastx results
cat $1 | xargs -I{fileID} sh -c 'cat bX_{fileID}/{fileID}*.blastx > Bacteroides-cellulosilyticus-WH2_vs_{fileID}.blastx'
...
cat $1 | xargs -I{fileID} sh -c 'perl /home3/acobian/bin/besthitblast.pl Bacteroides-cellulosilyticus-WH2_vs_{fileID}.blastx > besthit_Bacteroides-cellulosilyticus-WH2_vs_{fileID}.blastx'

#Count hits per database entry
cat $1 | xargs -I{fileID} sh -c 'cut -f 2 besthit_Bacteroides-cellulosilyticus-WH2_vs_{fileID}.blastx | sort | uniq -c | sort -nr  | sed -e "s/^ *//" | tr " " "\t" > vs_Bacteroides-cellulosilyticus-WH2.{fileID}.tab'
